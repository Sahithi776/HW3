#the output quotes are only displayed once, i.e. there should be no repeating quotes in the output.
...
def duplicate_list(l): 
    dlist = [] 
    [dlist.append(x) for x in l if x not in dlist] 
    return dlist

Updatedquery=' '.join(duplicate_list(query.split()))
print(query)

for i,quote in enumerate(data_list):
	found_at = quote.lower().find(Updatedquery.casefold())
	if( found_at >= 0):
		print("Found at ", i, "..."+quote[found_at:found_at+90], "...")
