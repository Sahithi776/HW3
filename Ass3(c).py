#If not results found, print "No results found"
...
for i,quote in enumerate(data_list):
	found_at = quote.lower().find(query.casefold())

if( found_at >= 0):
	     print("Found at ", i, "..."+quote[found_at:found_at+80], "...")
else:
     print( "no results found,please try another search criteria")
     
     
#def search_multiple_words(search_words):
  

#if any(x in data_list for x in search_words):
#out = [s for s in data_list if any(xs in s for xs in search_words)]
#print(out)
#else:
 #print( "no results found,please try another search criteria")
